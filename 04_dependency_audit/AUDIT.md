...
This check needs network access to reach the advisory database, so it does not run under `--network none` like the other experiments. That isolation rule exists to contain untrusted *model loading*, which executes code; auditing a `requirements.txt` executes nothing, so the rule does not apply here. `--no-deps` is passed because every requirement is pinned to an exact version, which is what that flag expects. The results still include packages the pinned ones depend on, not only the lines in the file, so transitive dependencies can show up too. Expect the first run to take a few minutes: pip-audit reaches the advisory database over the network for each package.
